...
Quantum hardware targets independent set problems on specific graph geometries~\cite{lucas2014, pichler2018}, quantum annealers target binary optimization~\cite{glover2019}, and commercial engines like Gurobi and CPLEX target integer linear programs.
...
The current library contains 27~problem types.
Three serve as \emph{compilation targets}, each backed by a class of specialized solvers:
\begin{itemize}
  \item \textbf{MIS} (Maximum Independent Set): target for Rydberg atom arrays.
...
\end{itemize}
Most problem types have a reduction path to ILP, directly or through intermediate types, enabling larger-scale solving via commercial optimizers.
