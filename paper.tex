...
The first challenge is forming a consensus.
...
Getting a distributed team to agree on and sustain a shared formalism across hundreds of contributions is notoriously difficult---style guides go unread, conventions drift, and individual contributors develop their own patterns.

Even if consensus is achieved, a second barrier remains: the people who know the reductions---mathematicians, physicists, computer scientists---are rarely the same people who can implement them in a systems language like Rust.
Without a path for domain experts to contribute the mathematical content alone, the project is bottlenecked on the small intersection of people who know both the theory and the implementation language.

Finally, the project needs a relentless maintainer.
Every reduction demands the same repetitive cycle: coding, testing, documentation, overhead validation, review.
Human energy is finite; maintainers burn out, graduate, change jobs, or lose interest.
The project needs someone who will execute the same verification loop for the 45th time with the same attention as the first---and who will be available indefinitely.

AI coding agents break through all three barriers.
They read the project specification on every invocation and never deviate from conventions, resolving consensus by design.
They execute a no-code pipeline in which a domain expert files a structured issue and agents handle the implementation, separating mathematical insight from programming skill.
And they run the same verification loop indefinitely without fatigue, serving as the relentless maintainer that no human can be.
But agents alone are not sufficient---their output must be constrained to match contributor-specified ground truth.
