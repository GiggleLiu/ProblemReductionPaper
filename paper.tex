...
The library is implemented in Rust as three crates: a \emph{core library} defining problem types, reduction rules, and graph infrastructure; a \emph{command-line interface} (\lstinline{pred}) for graph exploration, path search, and end-to-end solving; and \emph{procedural macros} for compile-time validation of reduction metadata.
A typical workflow creates a problem instance, finds the cheapest reduction path to a solver backend, and extracts the solution---all through the \lstinline{pred} CLI or the library API.

\subsection{Problem Types}\label{sec:graph-structure}

Every problem type implements the \lstinline{Problem} trait, which requires a uniform evaluation interface: given any configuration, return a score (\Cref{app:architecture}).
Each type defines its own \emph{size measures}---Satisfiability has \lstinline{num_vars}, \lstinline{num_clauses}, and \lstinline{num_literals}; MIS has \lstinline{num_vertices} and \lstinline{num_edges}; Factoring has \lstinline{num_bits_first} and \lstinline{num_bits_second}.
...
A reduction rule implements the \lstinline{ReduceTo<T>} trait, which requires both a forward map and an inverse map in a single implementation---an agent cannot compile a forward reduction without providing solution extraction (\Cref{app:architecture}).
...
The cost function is configurable: minimize the number of reduction steps, or minimize a specific output dimension such as \lstinline{num_vars}.
