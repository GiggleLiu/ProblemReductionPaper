...
---
  ...
  \end{tcolorbox}
  \end{minipage}
  \caption{Excerpts from two representative skills.
    \textbf{Left} (\texttt{propose}, advisor): the hard gate enforces no-code contribution; topology analysis guides experts toward strategic reduction-graph gaps; pre-validation checks four quality dimensions before the issue enters the pipeline.
    \textbf{Right} (\texttt{add-rule}, automation): the checklist encodes the mathematical knowledge every reduction requires; reference implementations prevent convention drift; the paper entry ensures visual verifiability by human reviewers.}
