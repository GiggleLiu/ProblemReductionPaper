...
The library is implemented in Rust as three crates: a \emph{core library} defining problem types, reduction rules, and graph infrastructure; a \emph{command-line interface} (\texttt{pred}) for graph exploration, path search, and end-to-end solving; and \emph{procedural macros} for compile-time validation of reduction metadata.
...
\begin{figure}[t]
  ...
  \caption{Library architecture.
    ...
    \textbf{Bottom}: procedural macros that validate overhead expressions and variant registrations at compile time (dashed arrows).}
...
Each type defines its own \emph{size measures}---Satisfiability has \texttt{num\_vars}, \texttt{num\_clauses}, and \texttt{num\_literals}; MIS has \texttt{num\_vertices} and \texttt{num\_edges}; Factoring has \texttt{num\_bits\_first} and \texttt{num\_bits\_second}.
Each type also declares its \emph{best-known algorithm complexity} in these measures (e.g., $O(1.1996^{n_v})$ for MIS).
