...
The reduction graph is implemented as a Rust library with three components: a \emph{core crate} that defines problem types, reduction rules, and graph infrastructure; a \emph{command-line interface} (\lstinline{pred}) that exposes the graph for exploration, path search, and end-to-end solving; and \emph{procedural macros} that validate reduction overhead expressions and variant registrations at compile time.
...
Each problem type defines its own \emph{size measures}---Satisfiability has \lstinline{num_vars}, \lstinline{num_clauses}, and \lstinline{num_literals}; MIS has \lstinline{num_vertices} and \lstinline{num_edges}; Factoring has \lstinline{num_bits_first} and \lstinline{num_bits_second}.
Each node in the graph stores these measures together with the problem's \emph{best-known algorithm complexity} expressed in them (e.g., $O(1.1996^{n_v})$ for MIS).
Each directed edge carries a \emph{reduction overhead}: a multivariate polynomial mapping the source's size measures to the target's.
For example, the Satisfiability $\to$ MIS reduction maps a formula with $n_l$ literals to a graph with $O(n_l)$ vertices and $O(n_l^2)$ edges (\Cref{fig:reduction-graph}).
...
The current graph contains 27~problem types and 56~directed edges: 45~hand-coded reduction rules plus 11~inferred from subtype relationships.
Three problems serve as \emph{compilation targets}, each backed by a class of specialized solvers:
\begin{itemize}
...
Maintaining the graph requires monitoring three structural properties (\Cref{fig:reduction-graph}).
A problem's \emph{NP-hardness is verified} whenever a directed path from 3-SAT reaches it---no external certificate is needed.
An \emph{orphan node} has no edges and cannot reach any solver, representing a gap to be filled.
A rule is \emph{redundant} when a composite path with equal or lower overhead already connects the same endpoints.
Redundant rules cause path explosion: QUBO and SpinGlass are connected by linear-overhead bidirectional reductions, so any new rule to QUBO whose overhead matches an existing path through SpinGlass doubles the number of equally-optimal routes without reducing cost.
The system checks each proposed rule against existing composite paths and accepts it only if it genuinely reduces the overhead.
