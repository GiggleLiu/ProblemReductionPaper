...
To bridge \emph{many} problems to \emph{many} solvers, we organize reductions as a \emph{directed graph}: each node is a problem type, and each directed edge is a verified reduction.
A problem~$A$ is \emph{reducible} to a problem~$C$ whenever a directed path connects them---for instance, $C \leftarrow B \leftarrow A$ in the diagram below.
\begin{center}
  \includegraphics[width=0.70\columnwidth]{figures/reduction-graph-concept.pdf}
\end{center}
Every edge carries a \emph{reduction overhead}---a polynomial mapping the source's input size to the target's.
Overheads compose along paths ($r_{C \leftarrow A} = r_{C \leftarrow B} \circ r_{B \leftarrow A}$), so the system can find the cheapest route from any problem to a solver backend.

The key property is \emph{compositionality}: independently contributed edges combine into longer chains without coordination.
...
This infrastructure is increasingly needed.
Physics-inspired solvers---graph neural networks at million-variable scale~\cite{Schuetz2022PhysicsGNN}, hybrid quantum-GNN methods for TSP~\cite{He2024QuantumTSP}---assume a QUBO or Ising formulation as input, precisely the transformation that a reduction graph provides.
