...
\begin{enumerate}
  ...
  \item A \emph{multilayer verification stack} that checks each reduction from type signatures through round-trip tests to agentic feature tests, ensuring agent output matches contributor-specified ground truth (\Cref{sec:bridge}).
  \item \emph{Skills}: versioned, composable workflow documents that encode project conventions and domain knowledge, so that every agent invocation follows the same process (\Cref{sec:method}).
\end{enumerate}
We formalize the class of projects where this approach applies as \emph{bridge problems} (\Cref{sec:bridge}) and present the reduction graph as a case study (\Cref{sec:graph}).
\Cref{sec:evaluation} provides quantitative evidence, \Cref{sec:related} surveys related work, and \Cref{sec:conclusion} discusses limitations and future directions.
