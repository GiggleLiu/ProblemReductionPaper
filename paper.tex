...
\caption{Skill architecture organized by audience.
  ...
  thirteen automation skills run without human intervention.}
...
The \emph{maintainer} uses three interactive skills:
...
The remaining thirteen skills are fully \emph{autonomous}:
\texttt{run-pipeline} drives an issue end-to-end from selection to merge;
\texttt{issue-to-pr} converts a validated issue into a pull request, invoking \texttt{add-model} or \texttt{add-rule} to generate the implementation;
...
Five of the eighteen are \emph{mentor} skills (tinted in \Cref{fig:skill-architecture}): they interactively guide their user, pausing for human judgment where creativity is required.
