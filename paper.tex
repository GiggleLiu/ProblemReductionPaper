...
\Cref{fig:library-architecture} shows the three-layer architecture.
The \emph{core library} (\Cref{fig:library-architecture}, middle) defines problem types, reduction rules, and a graph engine that composes them.
A \emph{symbolic engine} supports both reduction overhead and algorithm complexity as mathematical expressions---enabling symbolic composition along paths, asymptotic comparison for redundancy detection, and numeric evaluation for cost-based path search.
Two solvers operate on the graph: a brute-force enumerator for small instances (used in round-trip testing) and an ILP solver for larger-scale solving.
An example database stores canonical instances that serve as the single source of truth for tests, documentation, and the CLI (\Cref{fig:roundtrip-pipeline}).

Three user-facing interfaces sit above the core (\Cref{fig:library-architecture}, top): the \texttt{pred} CLI for graph exploration, path search, and end-to-end solving; the Rust library API; and an auto-compiled PDF manual containing formal definitions, reduction theorems with proof sketches, and worked examples for every implemented model and rule.

A compile-time validation layer (\Cref{fig:library-architecture}, bottom, dashed arrows) checks overhead expressions and type registrations before the code can compile, catching variable-name typos and malformed expressions at build time rather than at runtime.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/library-architecture.pdf}
  \caption{Library architecture.
    The core library (green) defines problem types, reduction rules, a graph engine, solvers, a symbolic engine for overhead and complexity expressions, and an example database.
    User interfaces (blue) expose the core via CLI, library API, and an auto-compiled PDF manual.
...
\subsection{Problem Types}\label{sec:graph-structure}

Every problem type implements the \texttt{Problem} trait, which requires a uniform evaluation interface: given any configuration, return a score (\Cref{app:architecture}).
...
The ILP solver backend is swappable via Cargo features: the default is the open-source HiGHS~\cite{Huangfu2018HiGHS}, with Coin-CBC, SCIP, and others available as drop-in alternatives.

\subsection{Reduction Rules}

A reduction rule implements the \texttt{ReduceTo<T>} trait, which requires both a forward map and an inverse map in a single implementation---an agent cannot compile a forward reduction without providing solution extraction (\Cref{app:architecture}).
...
Overheads compose symbolically along paths: the output expressions of one edge are substituted into the input variables of the next.

The library contains 56~directed edges: 45~hand-coded reduction rules plus 11~inferred from subtype relationships.
To route a problem instance to a solver, the system finds the cheapest reduction path using Dijkstra's algorithm.
...
Because overheads compose symbolically, the optimal path depends on the source instance's actual dimensions.

\subsection{Emergent Compositionality}\label{sec:compositionality}

The library's most valuable property is that independently implemented reductions compose automatically to solve problems that no single reduction was designed for.
