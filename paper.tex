...
\end{center}
In this graph, each node is a problem type annotated with its \emph{best-known algorithm complexity} and how it depends on the \emph{input size}; each directed edge is a verified reduction annotated with its \emph{reduction overhead}---a multivariate polynomial mapping the source's size to the target's.
A problem (e.g. $A$ in the diagram) is \emph{reducible} to a problem ($C$) whenever a directed path connecting them ($C \leftarrow B \leftarrow A$).
Overheads compose symbolically along paths (e.g. $r_{C \leftarrow A} = r_{C \leftarrow B} \circ r_{B \leftarrow A}$), so the system can find the cheapest route from any problem to a solver backend.
...
Physics-inspired solvers reinforce the need for such infrastructure: graph neural networks solve QUBO at million-variable scale~\cite{Schuetz2022PhysicsGNN}, and hybrid quantum-GNN methods tackle the Traveling Salesman Problem~\cite{He2024QuantumTSP}---but both assume a QUBO or Ising formulation as input, precisely the transformation that a reduction graph provides as upstream infrastructure.
