...
The library is implemented in Rust as three crates: a \emph{core library} defining problem types, reduction rules, and graph infrastructure; a \emph{command-line interface} (\texttt{pred}) for graph exploration, path search, and end-to-end solving; and a \emph{compile-time validation layer} that checks overhead expressions and type registrations before the code can compile.
A typical workflow creates a problem instance, finds the cheapest reduction path to a solver backend, and extracts the solution---all through the \texttt{pred} CLI or the library API.
The build system also produces an auto-compiled PDF manual containing formal definitions, reduction theorems with proof sketches, and worked round-trip test examples for every implemented model and rule.
...
\begin{figure}[t]
  ...
  \caption{Library architecture.
    \textbf{Top}: user-facing interfaces---the \texttt{pred} CLI, the Rust library API, and an auto-compiled PDF manual.
    \textbf{Middle}: the core crate with problem types, reduction rules, graph engine, solvers, and example database.
    \textbf{Bottom}: compile-time validation layer that checks overhead expressions and type registrations before compilation succeeds (dashed arrows).}
