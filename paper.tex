...
Our round-trip testing verifies reduction correctness automatically, but only on finite instances---it is not a formal proof.
Recent progress in AI-assisted theorem proving, where neural provers now produce machine-checked proofs at competition level, suggests that formally verifying each reduction in a proof assistant such as Lean is becoming feasible.
Integrating such guarantees would elevate the library from a tested software artifact to a certified reference for complexity theory.
