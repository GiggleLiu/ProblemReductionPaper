...
To tame this variance, \emph{harness engineering}~\cite{OpenAI2026HarnessEngineering,Fowler2026HarnessEngineering} shifts the engineer's role from code authorship to environment design: persistent project instructions~\cite{CursorRules2026,WindsurfRules2026,ClaudeCodeSkills2026}, composable skills~\cite{Obra2025Superpowers}, and verification pipelines constrain agents toward reliable output.
Our approach instantiates this pattern for a mathematical domain, exploiting the project's uniform, verifiable structure so that each agent invocation is short and self-contained.

\textbf{AI-assisted formal verification.}
Neural theorem provers have reached competition level---DeepSeek-Prover-V2~\cite{DeepSeekProverV2} scores 88.9\% on MiniF2F, and Harmonic's Aristotle~\cite{Harmonic2025Aristotle} solved 5/6 IMO~2025 problems with verified Lean~4 proofs---strengthening the argument that AI-generated code volumes will force machine-checkable correctness into the mainstream~\cite{Kleppmann2025FormalVerification}.
VeriCoding~\cite{Bursuc2025VeriCoding} benchmarks verified code generation, achieving 82\% in Dafny but only 44\% in Verus/Rust.
Our round-trip testing occupies a pragmatic middle ground: it verifies a mathematical property (reduction correctness) automatically, without a proof assistant, at the cost of covering finite instances rather than all inputs.
