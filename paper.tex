...
The graph can be strengthened by adding more primitive rules. For example, even if we already have a composite rule from $A$ to $C$, if a direct reduction from $A$ to $C$ has provable better overhead, it should be added as a primitive rule.
In contrast, if the new primitive does not improve the overhead, it should not be avoided to reduce the path finding complexity.

Two kinds of composite rules play a special role.
If a composite rule exists from 3-SAT to a problem~$H$, that path constitutes an NP-hardness proof for~$H$---we say $H$ is \emph{NP-hard verified} by the graph.
If a composite rule exists from a problem to ILP, that problem gains access to mature commercial solvers (Gurobi, CPLEX), providing an efficient verification oracle for testing reductions end-to-end.
