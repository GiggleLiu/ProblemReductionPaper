...
We call a single such reduction a \emph{primitive reduction rule}.
To bridge \emph{many} problems to \emph{many} solvers, we organize primitive rules as a \emph{directed graph}: each node is a problem type, each directed edge is a primitive rule, and each edge carries a \emph{reduction overhead}---a polynomial mapping the source's size measures to the target's.
\begin{center}
  \includegraphics[width=0.70\columnwidth]{figures/reduction-graph-concept.pdf}
\end{center}
Reductions are composable.
A \emph{composite reduction rule} is a directed path through the graph: chaining $A \to B$ and $B \to C$ yields the composite rule $A \to C$, with overhead $r_{C \leftarrow A} = r_{C \leftarrow B} \circ r_{B \leftarrow A}$.
When multiple composite rules connect a problem to different solvers, the system compares total cost---reduction overhead plus solver complexity---and selects the cheapest route for a given input size.
The graph coloring practitioner no longer chooses a solver; the graph finds the best path automatically.

Composite rules emerge without coordination.
One contributor implemented Factoring $\to$ CircuitSAT; another implemented CircuitSAT $\to$ ILP.
Neither intended the combination, yet the graph chains them to factor integers via linear programming.
Each new primitive rule amplifies the graph: adding a rule from problem~$X$ to MIS connects $X$ to every solver reachable from MIS, and every problem upstream of~$X$ to MIS.

Two composite-rule patterns recur throughout this paper.
First, if a composite rule exists from 3-SAT to a problem~$H$, that path constitutes an NP-hardness proof for~$H$---we say $H$ is \emph{NP-hard verified} by the graph.
Second, if a composite rule exists from a problem to ILP, that problem gains access to mature commercial solvers (Gurobi, CPLEX), providing an efficient verification oracle for testing reductions end-to-end.
