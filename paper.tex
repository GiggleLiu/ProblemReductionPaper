...
Building a reduction graph at scale is hard---not computationally, but organizationally.
...
The bottleneck is not mathematical knowledge but the cost of building and maintaining the software.

Two failure modes recur.
First, every edge must conform to the same conventions---same interface, same file layout, same test pattern---and human contributors inevitably accumulate their own patterns over time (\emph{convention drift}).
Second, every new entry demands the same lengthy checklist: confirm novelty, write a definition, prove correctness, implement forward and inverse maps, design tests, and document a worked example.
Repeating this cycle exhausts even dedicated maintainers (\emph{effort exhaustion}).
Worse, a mathematician who understands a new reduction has no way to contribute it without first learning the implementation language and the project's conventions (\Cref{fig:comic}).

AI agents offer a way forward---if their output can be trusted.
The recent ``harness engineering'' framing shifts the engineer's job toward designing the constraints, feedback loops, and verification systems that channel agents toward reliable output~\cite{OpenAI2026HarnessEngineering}.
The central requirement is a \emph{verification gate}: a cheap, automatic check that rejects incorrect output before it enters the codebase.
Reduction-graph construction supplies one for free---every reduction admits a \emph{round-trip test} (reduce, solve, map back, check optimality)---sidestepping the performance degradation agents exhibit on longer-horizon tasks~\cite{Thai2025SWEEVO, Deng2025SWEBenchPro}.
Together with a project \emph{scaffold} (repository structure, CI, specification files \texttt{CLAUDE.md}/\texttt{AGENTS.md}) and committed \emph{skills}~\cite{Claude2026AgentSkills}---plain-Markdown checklists that encode the project's conventions---this harness directly addresses both convention drift and effort exhaustion.
