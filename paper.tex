...
\emph{Toward machine-checked correctness.} Round-trip tests verify reduction correctness without a proof assistant. A natural next step is to route proofs through a formal system such as Lean and accept only those that typecheck. Whether current provers handle the combinatorial arguments reductions typically involve remains open, but integrating a proof assistant into the pipeline is, we think, the most promising direction for future work.
