...
\textbf{Automation.}
\texttt{run-pipeline} picks the highest-priority Ready issue from the project board and drives it through implementation to the review pool---fully autonomously, in a loop.
\texttt{issue-to-pr} converts a validated issue into an executable pull request: it researches references, writes a step-by-step plan, creates the PR, and invokes \texttt{add-model} or \texttt{add-rule} to generate the implementation.
\texttt{add-model} implements a new problem model end-to-end: Rust types, trait implementations, unit tests, CLI registration, and a paper entry.
\texttt{add-rule} does the same for a reduction rule, including the overhead expression, round-trip tests, and a proof sketch.
\texttt{review-pipeline} orchestrates three read-only sub-reviews in parallel:
\texttt{check-issue} validates issue quality (usefulness, non-triviality, correctness of literature claims);
\texttt{review-structural} verifies project-specific checklists, build success, and semantic correctness;
\texttt{review-quality} evaluates generic code quality (DRY, KISS, test coverage, cohesion).
\texttt{fix-pr} addresses CI failures, review comments, and coverage gaps without human input.
\texttt{topology-check} detects orphan problems, NP-hardness proof gaps, and redundant rules in the reduction graph.
\texttt{write-model} and \texttt{write-rule} draft the corresponding Typst paper entries with formal definitions, proof sketches, and worked examples.

Five of the eighteen---\texttt{tutorial}, \texttt{propose}, \texttt{dev-setup}, \texttt{final-review}, and \texttt{fix-issue}---are \emph{mentor} skills (tinted in \Cref{fig:skill-architecture}): they interactively guide their user, pausing for human judgment where creativity is required.
The remaining twelve run autonomously; \texttt{release} is triggered by the maintainer but executes without interaction.
