...
To bridge \emph{many} problems to \emph{many} solvers, we organize reductions as a \emph{directed graph}: each node is a problem type, each directed edge is a verified reduction, and each edge carries a \emph{reduction overhead}---a polynomial mapping the source's size measures to the target's.

\begin{center}
  \includegraphics[width=0.70\columnwidth]{figures/reduction-graph-concept.pdf}
\end{center}

Overheads compose along paths ($r_{C \leftarrow A} = r_{C \leftarrow B} \circ r_{B \leftarrow A}$).
...
The graph is also \emph{compositional}: independently contributed edges combine into chains without coordination.
One contributor implemented Factoring $\to$ CircuitSAT; another implemented CircuitSAT $\to$ ILP.
Neither intended the composition, yet the graph chains them to factor integers via linear programming.
Each new edge amplifies the graph: adding a reduction from problem~$X$ to MIS connects $X$ to every solver reachable from MIS, and every problem upstream of~$X$ to MIS.
