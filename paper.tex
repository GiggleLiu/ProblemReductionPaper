...
\caption{Skill architecture organized by audience.
  ...
  Seven \emph{mentor} skills (tinted) interactively guide a human;
  eleven automation skills run without human intervention.}
...
A \emph{user} invokes \texttt{tutorial} to explore the CLI without touching code.
A \emph{contributor}---typically a domain expert who need not know Rust---invokes \texttt{propose} to brainstorm a new model or reduction and file a GitHub issue.
The \emph{maintainer} uses five interactive skills:
\texttt{dev-setup} configures the toolchain,
\texttt{add-model} and \texttt{add-rule} guide step-by-step implementation,
\texttt{final-review} presents a structured merge-or-hold decision,
and \texttt{fix-issue} brainstorms fixes for failing quality checks.
The remaining eleven skills are fully \emph{autonomous}:
...
\texttt{fix-pr} addresses CI failures and review comments,
\texttt{topology-check} verifies reduction-graph consistency,
\texttt{write-model}/\texttt{write-rule} draft paper entries,
and \texttt{release} manages versioning.
Seven of the eighteen are \emph{mentor} skills (tinted in \Cref{fig:skill-architecture}): they interactively guide their user, pausing for human judgment where creativity is required.
