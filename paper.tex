...
\caption{Project knowledge architecture organized by audience.
  \texttt{CLAUDE.md} encodes conventions, architecture, and commands;
  eighteen skills serve three roles---user, contributor, and maintainer.
  Tinted boxes mark \emph{mentor} skills that interactively guide their user through multi-step workflows.}
...
\emph{User} skills lower the barrier to entry:
\texttt{propose} guides a domain expert through brainstorming a new model or rule and files a GitHub issue;
\texttt{tutorial} walks a newcomer through the CLI.
\emph{Contributor} skills automate routine development:
\texttt{dev-setup} configures the toolchain,
\texttt{add-model} and \texttt{add-rule} guide step-by-step implementation of new problems and reductions,
\texttt{issue-to-pr} converts a validated issue into an executable pull request,
\texttt{fix-issue} and \texttt{fix-pr} address review feedback,
and \texttt{write-model}/\texttt{write-rule} draft the corresponding paper entries.
\emph{Maintainer} skills handle orchestration and quality:
\texttt{run-pipeline} drives an issue end-to-end from selection to merge,
\texttt{review-pipeline} composes three sub-reviews (\texttt{check-issue}, \texttt{review-structural}, \texttt{review-quality}),
\texttt{final-review} presents a structured decision to the human maintainer,
\texttt{topology-check} verifies reduction-graph consistency,
and \texttt{release} manages versioning.
Six of the eighteen---\texttt{propose}, \texttt{tutorial}, \texttt{dev-setup}, \texttt{add-model}, \texttt{add-rule}, and \texttt{final-review}---are \emph{mentor} skills: they interactively guide their user, pausing for human judgment where creativity is required.
