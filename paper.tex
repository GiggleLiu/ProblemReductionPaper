...
Reduction-graph construction is a natural instance: every reduction admits a \emph{round-trip test}---reduce a small instance, solve the target by brute force, map the solution back, and check optimality---without a problem-specific oracle (\Cref{sec:roundtrip}).
...
Correctness assurance comes from four progressive verification layers (\Cref{fig:verification-funnel}).
Each layer catches a different class of error; no single layer suffices.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/verification-funnel.pdf}
  \caption{Verification funnel.
    Candidate contributions are progressively filtered: issue checks reject invalid proposals, tests reject implementation errors, agentic reviews reject behavioral issues, and contributor verification catches quality gaps.
    After merge, the domain expert verifies that the result matches their original intent.
    \color{red}{add example, focus on the lens of issue check, agentic tests.}
    }
  \label{fig:verification-funnel}
\end{figure}

\textbf{Layer~1: Issue check.}
Before any code is written, the proposal itself is validated: \emph{usefulness} (does a cheaper path already exist?), \emph{non-triviality} (is this a genuine structural transformation?), \emph{correctness} (do cited references support the claims?), and \emph{writing quality} (are all symbols defined, all examples fully worked?).
Invalid proposals are rejected before implementation effort is spent.

\textbf{Layer~2: Unit tests and round-trip tests.}
The type system catches structural errors at compile time: an agent cannot implement a reduction without providing solution extraction, reference nonexistent problem attributes, or omit required variant declarations.
Round-trip tests (\Cref{sec:roundtrip}) catch semantic errors---incorrect variable mappings, off-by-one indexing, forgotten negations---without per-reduction test logic.
Overhead validation compares symbolic size expressions against actual target sizes, catching formula errors that are type-correct but mathematically wrong.
Materialized fixtures address the ``lazy agent'' problem: ground-truth data is generated independently and committed separately, so tampering produces a visible diff in a file outside the agent's normal scope.

\textbf{Layer~3: Agentic feature tests.}
Open-source libraries normally stabilize through years of community use: real users with diverse backgrounds file bug reports, request clarifications, and expose edge cases that developers never anticipated.
...
\begin{figure}[t]
  ...
  \end{tcolorbox}
  \caption{A representative agent profile used in Layer~3 testing.
...
\textbf{Layer~4: Contributor verification.}
After merge, the domain expert who filed the original issue verifies the result.
This layer catches errors invisible to automated tests: whether the reduction matches the intent of the original proposal, whether the proof sketch is convincing, and whether the formal definition faithfully captures the problem.

The skill system ensures all four layers are invoked for every contribution.
