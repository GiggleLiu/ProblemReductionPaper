...
\emph{The evolving role of humans.} Harness engineering shifts where human effort is spent. In our work, none of the code was written manually; most effort went into the harness itself. This reallocation is what keeps an AI-coded project maintainable at scale: without a harness enforcing uniform conventions, a codebase grown at agent speed quickly becomes unmanageable. The harness is what makes the output of many short, disposable agent sessions add up to a coherent library rather than a pile of sediment.

\emph{Automation and advisor skills.} A harness that only automates is brittle. Our split follows a simple principle: delegate what the verifier can catch, keep a human in the loop for what it cannot. Humans are still needed where errors are expensive and hard to detect downstream. We expect this distinction to become standard vocabulary in harness design.

\emph{Toward machine-checked correctness.} Round-trip tests verify reduction correctness without a proof assistant, but leave the proof sketches themselves unverified. A natural next step is to route proofs through a formal system such as Lean and accept only those that typecheck. Whether current provers handle the combinatorial arguments reductions typically involve remains open, but integrating a proof assistant into the pipeline is, we think, the most promising direction for future work.
