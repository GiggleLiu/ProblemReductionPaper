...
Each simulated user is defined by an \emph{agent profile}: a persona with a backstory, expertise level, decision tendencies, and testing quirks.
For example, the profile ``Dr.\ Sarah Chen'' describes a computational physicist who evaluates optimization tools by running them against problems with known optima; she tests edge cases proactively (empty graphs, disconnected components), inspects JSON output with \texttt{jq}, and gets impatient with vague error messages.
The agent playing this role runs the full \texttt{create~$\to$~reduce~$\to$~solve~$\to$~evaluate} pipeline and flags any deviation from expected results---catching usability and correctness issues that unit tests miss.
