...
Persistent project instructions~\cite{CursorRules2026,WindsurfRules2026,ClaudeCodeSkills2026} and composable skills~\cite{Obra2025Superpowers} keep agents aligned with repository conventions across sessions; verification pipelines reject incorrect output before it enters the codebase.
...
agent session the same experimental conditions, while \emph{skills} and the \emph{automation pipeline}
turn recurring engineering work into a repeatable flow, unlocking a level of productivity that manual
workflows have not reached. At the same time, \emph{correctness assurance} remains the necessary
counterweight: with round-trip tests, agentic checks, and final human interpretation, the library stays
...
A large fraction of the engineering effort went into the repository architecture and the harness itself, rather than individual reductions.
The primary design goal was maintainability: a newcomer can read any single reduction in isolation, and a maintainer can modify the harness without understanding the mathematics of every rule.
Conventions are enforced by the harness, not by tribal knowledge, so the codebase remains navigable as it grows.

The same harness pattern extends naturally beyond reductions.
Any scientific computing domain that decomposes into uniform, independently testable tasks---automatic differentiation rules, compiler optimization passes, numerical quadrature schemes---can adopt a similar scaffold-and-skill structure.
We view the methodology not as specific to NP-hard problems but as a template for building scientific software with AI agents, one where the human effort goes into designing the harness rather than writing the code.

The current verification stack---round-trip tests on finite instances plus human review of proof sketches---catches implementation errors effectively but does not constitute a formal proof of correctness.
Strengthening the mathematical guarantees, for example by machine-checking reduction proofs in a proof assistant, is a natural next step that would raise the library's value as a reference for complexity theory.
We plan to pursue this in collaboration with domain experts and welcome community contributions through the existing issue-based route.

The reduction graph itself opens three further directions: \emph{fine-grained cost prediction} via a symbolic engine that selects the cheapest reduction path for a given instance; extension beyond NP to \emph{other complexity classes} (APX, PTAS, \#P) with preserved hardness guarantees; and \emph{automated discovery}, where the graph's topology identifies high-value missing edges and an agentic pipeline generates, validates, and absorbs new reductions.
