...
Three structural properties guide the graph's growth.
A problem's \emph{NP-hardness is verified} whenever a directed path from 3-SAT reaches it---no external certificate is needed.
An \emph{orphan node} has no edges and cannot reach any solver, representing a gap to be filled.
A rule is \emph{redundant} when a composite path with equal or lower overhead already connects the same endpoints; redundant rules cause combinatorial explosion of equally-optimal routes and are rejected automatically.
...
Each type defines its own \emph{size measures}---Satisfiability has \texttt{num_vars}, \texttt{num_clauses}, and \texttt{num_literals}; MIS has \texttt{num_vertices} and \texttt{num_edges}; Factoring has \texttt{num_bits_first} and \texttt{num_bits_second}.
...
The cost function is configurable: minimize the number of reduction steps, or minimize a specific output dimension such as \texttt{num_vars}.
...
This appendix describes four mechanisms---the \texttt{Problem} trait, the \texttt{ReduceTo} trait, the \texttt{#[reduction(overhead)]} macro, and the \texttt{declare_variants!} registry---that enforce correctness structurally (\Cref{fig:architecture}).
...
The generic \texttt{ReduceTo<T>} trait requires a \texttt{reduce_to()} method returning a \texttt{ReductionResult} that bundles \texttt{target_problem()} and \texttt{extract_solution()}.
...
These expressions are parsed at compile time; variable names are validated against getter methods on the source type, so a typo (e.g., referencing \texttt{num_vertex} when the method is \texttt{num_vertices}) causes a compile error rather than a silent bug.
...
The \texttt{declare_variants!} macro registers concrete instantiations with their best-known complexity, enabling automated graph export, documentation completeness checking, and redundancy analysis.
