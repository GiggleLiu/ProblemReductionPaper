...
\Cref{fig:reduction-graph} illustrates the idea: nodes are problem types, directed edges are reduction rules with overhead polynomials $r_{A \rightarrow B}$, and following a path through the graph leads from any supported problem to a solver backend with complexity $t_S$.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.80\columnwidth]{figures/reduction-graph-concept.pdf}
  \caption{Conceptual reduction graph.
  Nodes are problem types, directed edges are verified reductions annotated with overhead polynomials.
  Following a path leads from any supported problem to a solver backend.}
...
Physics-inspired solvers reinforce the need for such infrastructure: graph neural networks solve QUBO at million-variable scale~\cite{Schuetz2022PhysicsGNN}, and hybrid quantum-GNN methods tackle the Traveling Salesman Problem~\cite{He2024QuantumTSP}---but both assume a QUBO or Ising formulation as input, precisely the transformation that a reduction graph provides.
...
\textcolor{red}{Add typical using cases, like a mini tutorial for using pred.}
