...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\columnwidth]{figures/library-architecture.pdf}
...
\textbf{Automation skills} are plain-Markdown documents that orchestrate multi-step pipelines---connecting specification, tools, and knowledge into repeatable workflows.
Unlike traditional automation (Makefiles, shell scripts), which chains deterministic commands, skills can include \emph{abstract} steps: ``implement the reduction algorithm''(\texttt{run-pipeline} skill in~\Cref{fig:skill-architecture}), ``review the pull request for correctness''(\texttt{review-pipeline} skill)  and ``make a new release''(\texttt{release} skill).
...
\textbf{Advisor skills} more actively involve humans, for two purposes: \emph{training} humans to use the library or contribute to it, and \emph{utilizing} humans for tasks agents cannot reliably perform, such as critical decisions, hard mathematical reasoning, and creative work like designing canonical examples.
...
A catalog of reductions matters only if it changes outcomes when an agent tackles a concrete instance.
The most direct mechanism to test is whether catalog access lets the agent pick a downstream formulation it would not otherwise reach.
We probe this on dense weighted Max-Cut ($n{=}150$, ${\approx}5300$ signed edges), where the textbook ILP encoding is dominated by simulated annealing on QUBO: a regime in which catalog-aware reformulation has fair scope to pay off, so a null result here would refute the mechanism.
The instance is presented to a sub-agent in domain language with no algorithmic hints.
Four configurations vary two axes---\emph{web search} and \emph{access to \texttt{pred}}---across four random seeds (full setup in \Cref{app:end-user}).
We separate three quantities AI-coding evaluations often conflate as ``time'': AI tokens, solver wall-time, and solution quality, with optimality claims independently graded.

\Cref{tab:end-user-eval} summarises the outcomes.
Agents with \texttt{pred} access achieve zero regret on every seed and never falsely claim optimality.
Without it, agents leave 0.35--9.7\% on the table and produce 1--2 \emph{false-OPT} claims (an answer claimed optimal but beaten by another configuration on the same seed) out of four runs.
Web access alone is worse than neutral: it sharpens the agent's confidence in a textbook ILP encoding whose CBC time-out is then misreported as \texttt{OPTIMAL}.
With \texttt{pred}, the agent invariably reformulates Max-Cut as QUBO and solves it with simulated annealing, also winning on solver wall-time (42.8--71.1\,s vs.\ 81.8--85.3\,s).
The catalog-aware route costs 30--40\% more AI tokens for downstream enumeration---a cost the better answers and honest optimality reporting easily justify.
This points to the library's natural sweet spot: problems where outcome quality depends more on which formulation the solver sees than on tuning within a fixed formulation.

\begin{table}[h]
\caption{Outcomes on the Max-Cut instance (4~seeds per configuration; 90~s solver-wall cap).
Regret~\% is the gap to the best feasible answer found on the same seed; false-OPT counts answers claimed optimal but beaten by another configuration.}
\label{tab:end-user-eval}
\centering
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
Configuration & Mean regret \% & False-OPT & Mean AI tokens & Mean solver s \\
\midrule
Bare AI                              & 0.35 & 1/4 & 39{,}941 & 81.8 \\
Bare AI + web                        & 9.70 & 2/4 & 36{,}942 & 85.3 \\
+ \texttt{pred}                      & 0.00 & 0/4 & 55{,}332 & 71.1 \\
+ \texttt{pred} + web                & 0.00 & 0/4 & 51{,}109 & 42.8 \\
\bottomrule
\end{tabular}
\end{table}
...
---
  ...
  \caption{Two skills that bookend the contribution pipeline.
    \textbf{\texttt{propose}} (top) gates issue creation by domain experts: the hard gate enforces no-code contribution, topology analysis guides experts toward strategic reduction-graph gaps, and pre-validation checks four quality dimensions before the issue enters the pipeline.
    \textbf{\texttt{add-rule}} (bottom) drives implementation from a validated issue: the checklist encodes the mathematical knowledge every reduction requires, reference implementations prevent convention drift, and the paper entry ensures visual verifiability by human reviewers.}
  \label{fig:skills}
\end{figure}
...
\paragraph{Setup.}
A dense weighted Max-Cut instance ($n=150$ vertices, ${\approx}\,5300$ signed-weight edges, density ${\approx}\,0.48$) is presented to a Claude Sonnet 4.6 sub-agent through an engineer-voice prompt: a people-ops manager dividing an organization into two product teams to maximize cross-team interaction value.
The prompt contains no algorithm, model, or solver hints; the underlying Max-Cut structure is left for the agent to recognize.
Four configurations form a $2{\times}2$ factorial along two axes: \emph{web search} (yes/no) and \emph{access to \texttt{pred}} (yes/no).
Four random seeds per configuration; solver wall-time capped at 90~s per run.
Agents with \texttt{pred} access also receive a short Markdown protocol that prescribes one extra step before writing solver code---enumerate downstream models with \texttt{pred from} and pick the one whose endpoint admits the strongest off-the-shelf solver.
...
(iii) \emph{solution quality}---feasibility, objective value, and the agent's claim of optimality, all re-checked by an independent grader.
The grader also flags \emph{false-OPT}: a claimed-optimal answer beaten by another configuration on the same seed.

\paragraph{Caveats.}
A single scenario family in which the natural ILP formulation is known to be dominated by simulated annealing on the QUBO form---a setting deliberately chosen to test whether catalog-driven reformulation pays off when it can.
The gains may not transfer to families where the natural formulation is already strong (sparse ILP, scheduling), nor across different agent backends.
