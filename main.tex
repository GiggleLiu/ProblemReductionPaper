...
We test whether catalog access improves agent problem-solving on a domain-language instance: a HR manager splits a 150-person org into two product teams to maximize total signed cross-team value from a dense pairwise survey.
No abstract problem name is given; recognizing the structure and routing it to a competitive solver is part of the task.
Four configurations form a $2{\times}2$ factorial over web-search and \texttt{pred} access, ten random instances each (40 runs), with solver wall-time capped at 90~s.
We separate three quantities usually conflated as ``time'': AI tokens, solver wall-time, and graded solution quality (full setup in \Cref{app:end-user}).

\Cref{tab:end-user-eval} summarizes the outcomes.
\texttt{pred}-enabled agents achieve 0\% regret and zero false-OPT on all 10 instances; bare baselines leave 1.4--4.0\% on the table and produce 2/10 false-OPT each.
\texttt{pred} access also cuts solver wall-time (54.5--74.6\,s vs.\ 65.2--85.5\,s) at $\sim$35\% extra AI tokens.
Web access without \texttt{pred} is worse than no web at all: agents look up textbook ILP encodings and misread CBC's time-limit termination as \texttt{OPTIMAL}.

\textbf{Solver-choice analysis.}
\Cref{tab:end-user-solver-table} reports the model and solver each agent settled on.
Bare baselines scatter across five families (PuLP+CBC/HiGHS on a natural ILP encoding, Gurobi, OR-tools CP-SAT, dwave-neal, ad-hoc heuristics).
\texttt{pred}-enabled agents recognize the instance as signed-weight cut maximization on a dense graph, follow the catalog into the spin-glass / QUBO family, and run simulated annealing on the Ising form on every instance.
All four false-OPT incidents come from the ILP route.
The catalog points agents to a reformulation whose downstream solver is competitive on this class, and removes the per-run modelling variance.
...
\begin{table}[h]
\caption{Outcomes on the team-split instance class (10~instances per configuration; 90~s solver-wall cap; 40~runs total).
Regret~\% is the gap to the best feasible answer found on the same instance; false-OPT counts answers claimed optimal but beaten by another configuration.}
...
\paragraph{Setup.}
A Claude Sonnet 4.6 sub-agent receives an engineer-voice prompt: a HR manager splits a 150-person org into two product teams to maximize total cross-team value from a dense signed pairwise survey (${\approx}\,5300$ entries, density ${\approx}\,0.48$, mix of positive and negative scores).
...
Agents with \texttt{pred} access also receive a short Markdown protocol prescribing one extra step before writing solver code: enumerate downstream models with \texttt{pred from} and pick the one whose endpoint admits the strongest off-the-shelf solver.
...
The grader also flags \emph{false-OPT}: a claimed-optimal answer beaten by another configuration on the same instance.
...
Without \texttt{pred}, the bare baselines drift across PuLP+CBC, HiGHS, Gurobi, OR-tools CP-SAT, dwave-neal, and various custom local-search heuristics; with \texttt{pred}, every \texttt{pred}-enabled run converged to the same QUBO / simulated-annealing path.
...
\begin{table}[h]
\caption{Solver picked on each instance by each configuration. $\dagger$ marks false-OPT incidents.
SA~=~simulated annealing on the Ising/QUBO reformulation; Custom~=~hand-rolled local search / Kernighan-Lin / tabu / ILS.
Both \texttt{pred}-enabled configurations converge to dwave-neal SA but with different runtimes: web access lets the agent recover the package's documented hyperparameters and trust early stopping, while the offline variant defaults to a budget-filling loop.}
