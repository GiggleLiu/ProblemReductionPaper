...
A no-code contribution route that turns a structured GitHub issue into validated implementation, with no familiarity with the implementation language needed (\Cref{sec:method}).

\textbf{For maintainers.}
A skill-based agentic harness guides agents through the full pipeline, from coding to the final review, under a layered correctness gate.
Maintainers are freed from routine work to focus on high-risk decisions (\Cref{sec:skills}).

\textbf{For end users.}
The first comprehensive library of NP-hard reductions: 200+ reductions among 100+ problem types and 170k lines of code, an inventory that would have taken years by hand but landed in three months with a small team and AI agents.
Make complexity-theory reductions runnable: from a problem, route to a solver for an algorithm; from a solver, list every problem it can solve (\Cref{sec:end-user-utility}).
