...
\textbf{Advisor skills} more actively involve humans, for two purposes: \emph{training} humans to use or contribute to the library, and \emph{utilizing} humans for tasks agents cannot reliably perform---critical decisions, hard mathematical reasoning, and creative work like designing canonical examples.
End users invoke \texttt{find-solver} or \texttt{find-problem} (\Cref{fig:skill-architecture}) to navigate the reduction graph between problems and solvers; contributors use \texttt{propose} to elicit a new rule in mathematical language and file a well-formed issue without programming; maintainers use \texttt{fix-issue} and \texttt{final-review} to triage proofs and audit merges interactively.
...
A review agent reads the \texttt{review-pipeline} skill, claims an issue, and dispatches three read-only sub-agents covering structural completeness, code quality, and agentic feature testing (\Cref{sec:verification}).
It then posts a review report and moves the issue to \col{Final Review}---all headless.
...
We describe the two novel mechanisms below.
...
The pipeline is designed so that the mathematical intent propagates through all downstream artifacts; the contributor's final review confirms this (\Cref{fig:example-pipeline}).
...
\begin{figure}[t]
  ...
  \caption{Canonical examples as a single source of truth.
    JSON fixtures, the PDF manual, and \texttt{pred create -{}-example} are generated from the contributor's worked instance, so any drift surfaces as a visible mismatch.}
