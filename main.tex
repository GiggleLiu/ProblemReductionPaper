...
Correctness is enforced by five automated layers: issue review (Stage~2), compile-time type checks, unit tests, round-trip tests, and our \emph{agentic feature tests} (Stage~4)---a new layer we introduce and describe in detail below.

\textbf{Unit tests and round-trip tests.}
Unit tests target individual components: each model has tests for its evaluator and serialization, each solver has tests for correctness and witness recovery, and reduction helpers have their own assertions.
Round-trip tests target each reduction rule end-to-end: given a small canonical instance of the source problem, reduce through the rule, brute-force solve the target, lift the solution back to the source domain, and check it against an independent brute-force solve of the source---feasibility and objective value must agree.
The source-side brute force never invokes the rule, so each rule is graded against a value computed entirely outside it.
...
Beyond the four automated layers, the Stage~6 walkthrough with the original contributor remains as a final human safeguard, catching mismatches that automation cannot see---most often a builder that quietly encodes a different problem from the one in the issue.
