...
We evaluate end-user utility on dense weighted Max-Cut ($n{=}150$, $\approx 5300$ signed edges), an instance where direct ILP solving is dominated by reformulation to QUBO with simulated annealing.
...
We report three metrics that AI-coding evaluations often conflate as ``time'': AI tokens consumed, solver wall-time, and solution quality.
Optimality claims are graded independently against the best feasible solution found on the same seed.
Full setup is in \Cref{app:end-user}.
