...
name: propose
description:
  Guides domain experts through brain-
  storming and files a GitHub issue
---
  \end{lstlisting}
  \vspace{-4pt}
...
\subsection{Solver Coverage: QUBO}\label{sec:qubo-coverage}
...
%======================================================================
%  SECTION: END-USER UTILITY
%======================================================================
\section{End-User Utility of the Reduction Graph}\label{sec:end-user-utility}

The library's value to a practitioner is not the catalog of problems and reductions on its own, but whether having that catalog changes outcomes when a user---or an AI agent acting on their behalf---tackles a concrete instance.
We test this directly with a small controlled experiment.
...
\textbf{Three independent axes of measurement.}
...
\textbf{Results.}
Configurations with \texttt{pred} access achieve zero regret on all four seeds with no false-OPT claims; configurations without it leave 0.35--9.70\% on the table and produce 1--2 false-OPT claims out of 4 runs (\Cref{tab:end-user-eval}).
Web access alone is not merely neutral: the agent uses it to commit more confidently to a textbook ILP encoding, and the resulting CBC time-out is misreported as \texttt{OPTIMAL}, doubling the false-OPT rate and worsening mean regret to nearly $10\%$.
With \texttt{pred}, the agent invariably reformulates Max-Cut as a spin-glass / QUBO instance solved by simulated annealing (\texttt{dwave-neal}); this reformulation also wins on solver wall-time (42.8--71.1~s versus 81.8--85.3~s).
The catalog-aware path costs 30--40\% more AI tokens, primarily for enumerating downstream candidates; the trade is better answers and more honest optimality reporting.

\begin{table}[h]
\caption{Outcomes on a dense weighted Max-Cut instance (4~seeds per configuration; 90~s solver-wall cap; $n=150$ vertices, ${\approx}\,5300$ signed-weight edges).
Regret~\% is the gap to the best feasible answer found across all configurations on the same seed; false-OPT counts answers claimed optimal but beaten by another configuration.}
