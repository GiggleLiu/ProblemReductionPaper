...
Correctness is enforced by six layers: issue review (Stage~2), compile-time type checks, unit tests, round-trip tests, our \emph{agentic feature tests} (Stage~4)---a new layer we introduce and describe in detail below---and final review by a maintainer (Stage~5).
...
\textbf{Final review.}
Before merge (Stage~5), the \texttt{final-review} skill walks a maintainer through the agentic review report; the maintainer makes the merge call. This catches what earlier layers miss: most often a builder that quietly encodes a different problem from the one in the issue, or a judgment call where the automated review's confidence is misleading.

Beyond these six layers, Stage~6 brings the original contributor in for a community walkthrough: a domain-expert review against the original problem statement, catching subtle interpretation errors and proof gaps only the contributor recognizes.
