...
\textcolor{red}{Reduction-graph construction supplies one for free: every reduction admits a \emph{round-trip test}---reduce a small instance, solve the target problem, map the solution back, and check optimality.}
...
Correctness is enforced by six layers: issue review (Stage~2), compile-time type checks, unit tests, round-trip tests, agentic feature tests (Stage~4), and manual verification (Stage~6).
Five of these follow standard practice; we restate the round-trip pattern briefly because it is the verification gate the reduction-graph design supplies for free, and then describe the only mechanism we claim as novel, agentic feature tests.
...
For each reduction $A \to B$, an integration test exercises the registered solution lifter against this example: build $A$, reduce to $B$, solve $B$ by brute force, lift a witness back to $A$, and assert it is feasible against $A$'s own evaluator (and, when an externally computed Python fixture is available, equal to the canonical optimum).
The same builder feeds outside the test harness as well: the JSON fixtures, the Typst PDF manual's diagrams, and the CLI's \texttt{pred create -{}-example} mode (\Cref{fig:example-pipeline}) all share this single source.
The Stage~6 walkthrough then closes the loop on the human side, catching mismatches that the automated oracle cannot see (e.g., a builder that quietly encodes a different problem from the one in the issue).
...
\begin{figure}[t]
  ...
  \caption{Canonical examples as a single source of truth.
    ...
    The Stage~6 walkthrough closes the loop: the contributor checks the manual and CLI output against the original issue.}
