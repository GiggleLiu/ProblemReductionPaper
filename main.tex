...
The library produced by our harness is itself an agent-callable tool through the \texttt{pred} CLI.
We test whether catalog access measurably improves agent problem-solving on a representative instance: dense weighted Max-Cut ($n{=}150$, $\approx 5300$ signed edges), where direct ILP solving is dominated by reformulation to QUBO with simulated annealing.
The instance is presented to a sub-agent in domain language without algorithmic hints.
We compare four configurations along two axes---web search access and \texttt{pred} (catalog) access---on ten random seeds each, for 40 runs total.
We track three metrics that AI-coding evaluations often conflate as ``time'': AI tokens consumed, solver wall-time, and solution quality.
Optimality claims are graded independently against the best feasible solution found on the same seed (full setup in \Cref{app:end-user}).

\Cref{tab:end-user-eval} summarizes the outcomes.
Agents with \texttt{pred} access achieve 0\% regret on all 10 seeds and produce no false-OPT claims.
Agents without \texttt{pred} leave 1.4--4.0\% on the table on average and produce 2 false-OPT claims out of 10 runs in each baseline (a false-OPT being an answer claimed optimal but beaten by another configuration on the same seed).
\texttt{pred} access also reduces solver wall-time (54.5--74.6\,s versus 65.2--85.5\,s) at the cost of $\sim$35\% additional AI tokens.
Web access without \texttt{pred} degrades performance: it increases agent confidence in the textbook ILP encoding, whose CBC timeout is misreported as \texttt{OPTIMAL}.

\textbf{Solver-choice analysis.}
\Cref{tab:end-user-solver-table} reports each agent's solver selection.
Without \texttt{pred}, agents distribute across five solver classes (PuLP+CBC, HiGHS, Gurobi, OR-tools CP-SAT, dwave-neal, plus ad-hoc heuristics).
With \texttt{pred}, agents converge on simulated annealing applied to the QUBO reformulation across all 10 seeds in both \texttt{pred}-enabled configurations.
All four false-OPT incidents originate from PuLP+CBC (3) or PuLP+HiGHS (1), whose timeout behavior is reported as \texttt{OPTIMAL} rather than as an open optimality gap.
The catalog therefore contributes along two dimensions: it identifies the stronger reformulation, and it reduces inter-run variance arising from solver selection.

Beyond the Max-Cut instance, this experiment completes a loop that runs through the paper.
AI agents built the library under our harness pipeline; the library, deployed as an agent-callable tool, now augments the problem-solving of agents tackling new instances.
The harness thus produces tools that strengthen the systems that built them.
