...
Correctness is enforced by five layers: issue review (Stage~2), compile-time type checks, unit and integration tests, agentic feature tests (Stage~4), and manual verification (Stage~6).
The first three and the last are standard; agentic feature tests, the only mechanism we claim as novel, are described below.

\textbf{Canonical examples as the test oracle.}
A prerequisite for the pipeline is that every issue carries a small, fully worked instance with a known solution.
This canonical example feeds the example database as a builder function.
For each reduction $A \to B$, an integration test then exercises the registered solution lifter using this example: build $A$, reduce to $B$, solve $B$ by brute force, lift a witness back to $A$, and assert it is feasible against $A$'s own evaluator (and, where a Python-computed fixture is available, optimal).
The same builder also feeds the JSON fixtures, the Typst PDF manual's diagrams, and the CLI's \texttt{pred create -{}-example} mode (\Cref{fig:example-pipeline}), so the contributor's mathematical intent reaches every downstream artifact through a single source.
The walkthrough at Stage~6 closes that loop on the human side.
