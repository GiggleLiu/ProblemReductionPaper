...
A no-code contribution route that turns a structured GitHub issue into a merged reduction, with no familiarity with the implementation language needed (\Cref{fig:overview}(b)).

\textbf{For maintainers.}
A skill-based agentic harness in which agents execute the per-reduction checklist (coding, testing, documentation) under a layered correctness gate.
Maintainer effort no longer scales with library size (\Cref{sec:skills}).

\textbf{For end users.}
A library of 200+ executable reductions among 100+ problem types, accessible through a single command, so that any source problem can be translated into a form an off-the-shelf solver (SAT, ILP, QUBO, \dots) accepts.
170k lines of code, built in about three months by a small team and AI agents (\Cref{sec:end-user-utility}).
