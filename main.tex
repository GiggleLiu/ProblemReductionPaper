...
\begin{figure}[t]
  ...
  \caption{Library architecture.
    Three layers stack: interface (top), library (middle), and infrastructure (bottom).
    The interface layer exposes the reduction graph through the \texttt{pred} command-line tool and a PDF manual.
    The library layer registers problem types, reduction rules, and an example database.
    The infrastructure layer provides solvers for round-trip testing and a symbolic engine for overhead analysis.}
  \label{fig:library-architecture}
\end{figure}
