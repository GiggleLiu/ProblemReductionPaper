...
\Cref{tab:end-user-eval} summarizes the outcomes; \Cref{fig:end-user-scatter} plots all 40 runs in the (solver wall-time, objective) plane.
...
Each marker is one (configuration, instance) pair; horizontal dashed lines mark the per-instance best objective achieved by any configuration on that instance.
Cross markers ($\times$) indicate catastrophic misses where the agent's chosen solver finished below the per-instance best.
Catalog-enabled runs (\textcolor[HTML]{1f77b4}{\textbf{+pred}}, \textcolor[HTML]{2ca02c}{\textbf{+pred+web}}) cluster in the upper-left ``fast and on-target'' regime; the bare baselines (\textcolor[HTML]{d62728}{\textbf{Bare AI}}, \textcolor[HTML]{ff7f0e}{\textbf{Bare AI+web}}) spread further right and account for all but one of the catastrophic misses.}
