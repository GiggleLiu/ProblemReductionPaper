...
\textcolor{red}{Reduction-graph construction supplies one for free: every reduction admits a \emph{closed-loop unit test}---reduce a small canonical instance, solve the target, lift the solution back, and compare against an independent direct solve of the source.}
...
The harness rests on three components:
\begin{enumerate}
  ...
  \item A \emph{multilayer verification stack} that ensures correctness at every stage: \textcolor{red}{automated unit tests validate each reduction programmatically by exercising the solution lifter against an independent brute-force solve}, while domain experts perform end-to-end checks against contributor-specified ground truth (\Cref{sec:verification}).
\end{enumerate}
...
The crate ships with a command-line interface (CLI), \texttt{pred}, as the primary entry point.
The three commands compose through Unix pipes: \texttt{create} emits a problem instance as JSON, \texttt{reduce} transforms it to a target formulation, and \texttt{solve} invokes a solver. See Appendix~\ref{app:cli} for a worked CLI session and the full subcommand list.
...
The library layer (\Cref{fig:library-architecture}) registers the reduction graph itself: problem types, reduction rules between them, and a database of canonical instances for testing and documentation.
...
\textcolor{red}{The same instance feeds the unit tests, worked examples in the PDF manual, and the \texttt{pred create --example} command for interactive exploration.}
...
\emph{Solvers.}
\textcolor{red}{Every problem type needs a minimal but reasonably efficient solver, because the closed-loop unit tests rely on brute-forcing both the source and the target to validate each reduction.}
...
\textcolor{red}{Correctness is enforced by a four-layer wrapper around the codebase: issue review (Stage~2), compile-time type checks, unit tests, and agentic feature tests (Stage~4).
The first three are standard practice; agentic feature tests are the only mechanism we claim as novel.}

\textcolor{red}{For unit tests, the reduction-graph design supplies a verification gate for free.
Every issue carries a small canonical example with a known solution, encoded in the example database as a builder function.
For each reduction $A \to B$, the unit test then exercises the registered solution lifter against this example: build $A$, reduce to $B$, brute-force the target, lift each witness back to $A$, and compare against an independent brute-force solve of $A$ itself---asserting that the lifted set is contained in $A$'s optima and that objective values agree.
The same builder feeds outside the test harness as well: the JSON fixtures, the Typst PDF manual's diagrams, and the CLI's \texttt{pred create -{}-example} mode (\Cref{fig:example-pipeline}) all share this single source, so the contributor's mathematical intent reaches every artifact through one definition.}
...
\textcolor{red}{Beyond these four automated layers, the Stage~6 walkthrough by the original contributor serves as the final safety net: subtle misinterpretations of the problem statement or proof arguments---e.g., a builder that quietly encodes a different problem from the one in the issue---only surface when the contributor sees the rendered manual and CLI output side-by-side with the original issue.}
...
Four configurations vary two axes---\emph{web search} and \emph{access to \texttt{pred}}---across ten random seeds (40~runs total; full setup in \Cref{app:end-user}).
...
Agents with \texttt{pred} access achieve zero regret on every one of the 10~seeds and never falsely claim optimality.
Without it, agents leave 1.4--4.0\% on the table on average and produce 2 \emph{false-OPT} claims (an answer claimed optimal but beaten by another configuration on the same seed) out of 10~runs in each baseline.
...
With \texttt{pred}, the agent invariably reformulates Max-Cut as QUBO and solves it with simulated annealing, also winning on solver wall-time (54.5--74.6\,s vs.\ 65.2--85.5\,s).
The catalog-aware route costs ${\approx}\,35\%$ more AI tokens for downstream enumeration---a cost the better answers and honest optimality reporting easily justify.
The library is therefore most useful where the obvious encoding is suboptimal but a stronger one exists via reduction.

\textbf{Where the regret comes from: solver-choice variance.}
Tracking which solver each agent picked exposes the underlying mechanism (full per-seed picks in \Cref{tab:end-user-solver-table}).
Without \texttt{pred}, the bare AI scatters across five distinct solver classes (PuLP+CBC, HiGHS, Gurobi, OR-tools CP-SAT, dwave-neal, plus custom heuristics)---five different choices on 10~seeds for both A and B.
With \texttt{pred}, the agent converges to the same path on every seed: simulated annealing on the QUBO reformulation (10/10 for both C and D).
All four false-OPT incidents involve PuLP+CBC (3) or PuLP+HiGHS (1), whose timeout behaviour is misreported as \texttt{Optimal} rather than as an open optimality gap; agents that happened to pick Gurobi or simulated annealing avoid the misreport even when their answer is suboptimal.
The skill's value is therefore not only in finding a stronger formulation but also in eliminating the run-to-run variance that comes from the bare AI's solver lottery.

\begin{table}[h]
\caption{Outcomes on the Max-Cut instance (10~seeds per configuration; 90~s solver-wall cap; 40~runs total).
...
\midrule
Bare AI                              & 1.39 & 2/10 & 40{,}142 & 85.5 \\
Bare AI + web                        & 4.04 & 2/10 & 39{,}103 & 65.2 \\
+ \texttt{pred}                      & 0.00 & 0/10 & 54{,}637 & 74.6 \\
+ \texttt{pred} + web                & 0.00 & 0/10 & 53{,}849 & 54.5 \\
\bottomrule
\end{tabular}
\end{table}
...
safeguard: \textcolor{red}{with unit tests, agentic feature tests, and final human interpretation, the library stays
...
\textcolor{red}{Brute force remains essential for the closed-loop unit tests that validate every reduction, where correctness matters more than speed.}
...
Ten random seeds per configuration (40 runs total); solver wall-time capped at 90~s per run.
...
\paragraph{Per-seed solver picks.}
\Cref{tab:end-user-solver-table} lists the solver each agent selected on every seed.
Without \texttt{pred}, the bare baselines drift across PuLP+CBC, HiGHS, Gurobi, OR-tools CP-SAT, dwave-neal, and various custom local-search heuristics; with \texttt{pred}, every C and D run converged to the same QUBO/SA path.
False-OPT incidents are marked $\dagger$ and cluster on PuLP+CBC and PuLP+HiGHS, whose ``Optimal'' status under timeout is, in fact, a feasibility incumbent with an unclosed gap.

\begin{table}[h]
\caption{Solver picked on each seed by each configuration. Cells marked $\dagger$ are false-OPT incidents (the agent declared optimality but another configuration found a strictly better answer on the same seed).
SA = simulated annealing on the Ising/QUBO reformulation. Custom = hand-rolled local search / Kernighan-Lin / tabu / iterated local search.}
...
\midrule
 7 & PuLP+CBC$^\dagger$       & PuLP+CBC$^\dagger$    & dwave-neal SA & dwave-neal SA \\
11 & Gurobi + 1-flip          & Gurobi                & dwave-neal SA & dwave-neal SA \\
13 & PuLP+Gurobi              & PuLP+CBC$^\dagger$    & dwave-neal SA & dwave-neal SA \\
17 & custom (KL)              & custom (tabu)         & dwave-neal SA & dwave-neal SA \\
19 & custom (KL+ILS)          & Gurobi MIQP           & dwave-neal SA & dwave-neal SA \\
23 & HiGHS + local search     & dwave-neal SA         & dwave-neal SA & dwave-neal SA \\
29 & PuLP+HiGHS$^\dagger$     & dwave-neal SA         & dwave-neal SA & dwave-neal SA \\
31 & Gurobi (ILP)             & Gurobi                & dwave-neal SA & dwave-neal SA \\
37 & HiGHS                    & OR-tools CP-SAT       & dwave-neal SA & dwave-neal SA \\
41 & dwave-neal SA + polish   & dwave-neal SA         & dwave-neal SA & dwave-neal SA \\
\midrule
\textbf{distinct} & 5 classes & 5 classes & \textbf{1 class} & \textbf{1 class} \\
\bottomrule
\end{tabular}
\end{table}
