...
AI coding agents offer a way forward, but only when paired with a reliable \emph{verification gate}---a cheap, automatic check that rejects incorrect output before it enters the codebase~\cite{OpenAI2026HarnessEngineering}.
...
Building on this, we design a domain-specific harness that addresses both \emph{convention drift} and \emph{effort exhaustion}; we detail it in \Cref{sec:method}.
...
\textcolor{red}{The same instance feeds tests, worked examples in the PDF manual, and the \texttt{pred create --example} command for interactive exploration.}
