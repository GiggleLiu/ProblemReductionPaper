...
\begin{wrapfigure}{r}{0.4\textwidth}
  ...
  \caption{The reduction graph.
    Nodes are problem types; edges are primitive reductions labeled by overhead.
    Highlighted anchors show paths from 3-SAT for NP-hardness and paths to ILP for solver access.}
...
\begin{figure}
  ...
  \caption{Overview.
    (a) A reduction graph routes hard problems to multiple solver backends.
    (b) A harness-engineered workflow turns structured issues into validated code.
    (c) The resulting library grows rapidly once the automated pipeline is in place.}
  \label{fig:overview}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{System architecture.
    The interface layer exposes user and agent entry points; the library layer stores the reduction graph; the infrastructure layer supplies solvers and symbolic complexity analysis.}
...
\begin{wrapfigure}{r}{0.45\textwidth}
  ...
  \caption{Delegation spectrum. Tasks are ordered from human-led to agent-led; bands show the human--agent split.}
...
\begin{figure*}[htbp]
  ...
  \caption{Contribution pipeline as a GitHub project-board state machine.
    Boxes are board columns, and arrows show the allowed transition direction.
    Colors indicate the responsible role at each stage.}
  \label{fig:pipeline}
\end{figure*}
...
A domain expert clones the repository and invokes the \texttt{propose} skill (\Cref{fig:skills}) in Claude Code or a similar agentic coding tool.
...
The pipeline is designed so that the mathematical intent propagates through all downstream artifacts; the contributor's final review confirms this (see \Cref{app:example-pipeline}).
...
\section{Canonical Example Pipeline}\label{app:example-pipeline}

\begin{figure}[h]
  \centering
  \includegraphics[width=\columnwidth]{figures/roundtrip-pipeline-fig5-style.pdf}
...
\Cref{fig:skills} shows two representative skills: \texttt{propose}, an advisor skill that gates issue creation by domain experts, and \texttt{add-rule}, an automation skill that drives implementation from a validated issue.
...
\begin{figure}[h]
  ...
  \vspace{0pt}
  \begin{tcolorbox}[
    colback=blue!3, colframe=blue!65!black, colbacktitle=blue!12,
    coltitle=blue!55!black, equal height group=skill-excerpts,
  ...
  \footnotesize
  \begin{lstlisting}[basicstyle=\ttfamily\tiny, numbers=none, frame=none, aboveskip=0pt, belowskip=0pt, xleftmargin=2pt]
---
name: propose
description:
...
---
  ...
  Work entirely in mathematical language.\\[3pt]
  %
  \texttt{<HARD-GATE>}\\
  Do NOT write code or invoke implementation skills.
  The ONLY output is a GitHub issue.\\
  \texttt{</HARD-GATE>}\\[3pt]
  %
  \rule{\linewidth}{0.3pt}\\[3pt]
  \textbf{Run topology analysis first} to find high-impact gaps [\ldots]\\
  \begin{lstlisting}[basicstyle=\ttfamily\tiny, numbers=none, frame=none, aboveskip=-5pt, belowskip=2pt, xleftmargin=2pt]
cargo run --example detect_isolated_...
cargo run --example detect_unreachable_...
  \end{lstlisting}
  \vspace{-2pt}
  \rule{\linewidth}{0.3pt}\\[3pt]
  \textbf{Pre-validation} --- Run \texttt{check-issue}:\\[2pt]
  \begin{tabular}{@{}r@{~}l@{}}
    1. & \textbf{Usefulness} \\
    2. & \textbf{Effort} \\
    3. & \textbf{Correctness} \\
    4. & \textbf{Writing quality} \\
  \end{tabular}\\[2pt]
  File only issues passing all checks.
  \end{tcolorbox}
  \end{minipage}
  \hfill
  \begin{minipage}[t]{0.48\textwidth}
  ...
  \begin{tcolorbox}[
    colback=white, colframe=gray!80, colbacktitle=gray!15,
    coltitle=black, equal height group=skill-excerpts,
...
---
  ...
  \begin{enumerate}
    \item \textbf{Reduction algorithm} with reference.
    \item \textbf{Size overhead} for routing.
    \item \textbf{Concrete example} for tests and docs.
    \item $\ldots$
  \end{enumerate}
  \endgroup
  ...
  Write the theorem, proof, and worked example.
  Run \texttt{make paper}.
  \end{tcolorbox}
  \end{minipage}
  \caption{Skill excerpts for no-code issue creation (\texttt{propose}) and rule implementation (\texttt{add-rule}).}
  \label{fig:skills}
\end{figure}
