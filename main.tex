...
\begin{wrapfigure}{r}{0.42\textwidth}
  ...
  \caption{The reduction graph.  Two anchor problems are highlighted: \emph{3-SAT} at the top (source of NP-hardness proofs) and \emph{ILP} at the bottom right (gateway to mature solvers).  Edge labels are reduction overheads $r_{\cdot \leftarrow \cdot}$; node labels like $t_S$, $t_I$ denote the time complexity of the node's best known solver.}
  \label{fig:reduction-graph-concept}
  \vspace{-\baselineskip}
\end{wrapfigure}

A single reduction between two problems is a \emph{primitive reduction rule}.
To bridge \emph{many} problems to \emph{many} solvers, we organize primitive rules as a directed \emph{reduction graph} (\Cref{fig:reduction-graph-concept}).
Each node is a problem type; each directed edge is a primitive rule.
Every edge carries a \emph{reduction overhead}: a multivariate polynomial mapping the source's size measures to the target's; we write $r_{B \leftarrow A}$ for the overhead of the rule $A \to B$, as labeled on the middle edge of the figure.
...
A path from the well-established NP-complete problem \emph{3-SAT} (highlighted, top) to a problem~$H$ provides an NP-hardness argument for~$H$, backed by tested code rather than a pen-and-paper proof; we say $H$ is \emph{NP-hard certified} (see the node labeled ``NP-hard certified'' in the figure).
A path from a problem to \emph{integer linear programming} (ILP; highlighted, bottom right) grants access to mature solvers---crucial for testing reduction correctness, since the target's solver cost~$t_I$ upper-bounds the end-to-end cost of any problem routed through it.
