...
Four configurations form a $2{\times}2$ factorial over web-search and \texttt{pred} access, with ten random instances each (40 runs) and solver wall-time capped at 90~s.
For each run we record three quantities the literature often blurs together: AI tokens spent on modelling, solver wall-time, and the objective value graded against the per-instance best (full setup in Appendix~\ref{app:end-user}).
...
With the catalog, every run reformulates the instance as a signed-weight cut on a dense graph, follows the path through the spin-glass / QUBO family, and runs simulated annealing on the Ising form (one run picks a parallel-tempering variant).
