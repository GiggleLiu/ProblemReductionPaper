...
Does a catalog of reductions change what agents do? On dense weighted Max-Cut, yes: catalog access produces strictly better solutions, honest optimality reporting, and tight solver convergence.

We pick Max-Cut ($n{=}150$, $\approx 5300$ signed edges) because the textbook ILP encoding is known to be dominated by simulated annealing on a QUBO reformulation---a regime where a reduction catalog should help.
The instance is presented to a sub-agent in domain language only, with no algorithmic hints.
Four configurations vary two axes (web search and \texttt{pred} access) over ten random seeds each, for 40 runs total, separating three quantities AI evaluations often conflate: AI tokens, solver wall-time, and solution quality, with optimality claims independently graded (full setup in \Cref{app:end-user}).

\Cref{tab:end-user-eval} reports the outcomes.
Agents with \texttt{pred} reach 0\% regret on all 10 seeds and never falsely claim optimality.
Without \texttt{pred}, agents leave 1.4--4.0\% on the table on average and falsely claim optimality in 2 of 10 runs in each baseline.
\texttt{pred} also wins on solver wall-time (54.5--74.6\,s vs.\ 65.2--85.5\,s).
It costs $\sim$35\% more AI tokens, which the better answers and honest reporting justify.
Web access alone is actively harmful: it sharpens the agent's confidence in the textbook ILP encoding, whose CBC time-out is then misreported as \texttt{OPTIMAL}.

\textbf{Mechanism: from solver lottery to deterministic path.}
Tracking which solver each agent picked (\Cref{tab:end-user-solver-table}) shows what the catalog removes.
Without \texttt{pred}, the bare AI scatters across five solver classes (PuLP+CBC, HiGHS, Gurobi, OR-tools CP-SAT, dwave-neal, plus custom heuristics).
With \texttt{pred}, every seed converges to the same path: simulated annealing on the QUBO reformulation.
All four false-OPT incidents involve PuLP+CBC or PuLP+HiGHS, whose timeout behavior is misreported as \texttt{OPTIMAL}.
The catalog's value is therefore twofold: it surfaces the stronger reformulation, and it eliminates the run-to-run variance from the bare AI's solver guessing.

The result is strongest where the obvious encoding is suboptimal but a stronger one is available via reduction.
Outside this regime, the catalog adds tokens without adding options.
