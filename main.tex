...
The protocol scripts a four-phase workflow. Phase~1 reads the engineer-voice prompt and matches the natural model from \texttt{pred list}. Phase~2---the structural step that the bare AI does not perform---enumerates reachable downstream models (\texttt{pred from --hops 2}), enumerates all reduction paths with per-step overhead (\texttt{pred path --all}), saves the smallest-endpoint path (\texttt{pred path --cost minimize:<field> -o path.json}), reads each endpoint's worst-case complexity (\texttt{pred show}), and researches off-the-shelf solver capabilities for the candidate endpoints. Phase~3 builds the natural-model JSON, forward-reduces along the saved path (\texttt{pred reduce --via path.json}), runs a single-pass solver call, and inverse-maps the target-space configuration back to source-space (\texttt{pred extract}). Phase~4 runs an independent verifier that recomputes the objective from the original prompt literals; the agent reports the answer only after the verifier passes.
...
\begin{table}[h]
\caption{Raw per-instance objective and solver wall-time for each configuration. Each cell shows ``objective\,/\,wall-time\,(s)''; bold objectives fall below the per-instance best (right column). The 90~s OS-enforced kill cutoff bounds wall-times; entries near 80--90~s indicate the solver was running until killed.}
