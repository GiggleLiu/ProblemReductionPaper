...
\Cref{tab:end-user-eval} summarizes the outcomes: catalog access raises Best-match counts and cuts solver wall-time, at the cost of more AI tokens.
The bare-baseline and intermediate configurations all show large misses (bare AI's worst run drops 9.8\% below the per-instance best), traceable to a single failure mode: the agent commits to a textbook integer-programming formulation, whose linear relaxation is too loose to close within 90~s.
The extra tokens are spent reading the catalog; the speedup comes from the catalog's recommended endpoint, simulated annealing on the Ising form, which terminates in a single short call rather than a long branch-and-bound run.
