...
The CLI \texttt{pred} provides a friendly interface for both human users and AI agents.
They browse the catalog of problems and reductions, \texttt{create} a problem instance as JSON, and \texttt{reduce} it through any path in the graph; the resulting JSON flows to external solvers or downstream tools through Unix pipes.
A built-in \texttt{solve} closes the loop for quick checks.
See Appendix~\ref{app:cli} for a worked session.

The PDF manual is the human-readable specification: each problem and reduction comes with a formal definition, a worked example, a diagram, and a proof sketch.
Domain experts read it to verify that the implementation matches the mathematics.
