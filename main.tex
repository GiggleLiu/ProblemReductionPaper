...
We test whether catalog access improves agent problem-solving on a domain-language instance: a HR manager splits a 150-person organization into two product teams to maximize total signed cross-team value from a dense pairwise survey.
...
We separate three quantities usually conflated as ``time'': AI tokens, solver wall-time, and graded solution quality (full setup in Appendix~\ref{app:end-user}).

\Cref{tab:end-user-eval} summarizes the outcomes; \Cref{fig:end-user-scatter} plots all 40 runs in the (solver wall-time, gap-from-best) plane.
The full \texttt{pred}+web configuration matches the per-instance best objective on all 10 instances and is the only configuration whose minimum objective stays at the per-instance best.
The bare-AI baseline matches the per-instance best on 7 of 10 instances; the worst miss (instance 13, 11{,}673 versus 12{,}944) is a 9.8\% gap.
The two intermediate configurations each suffer one or two large misses where the agent commits to a textbook integer-programming encoding and the solver hits the wall-time cap with a poor incumbent: bare-AI+web finishes at 10{,}011 / 10{,}252 on instances 11 and 19; \texttt{pred} (offline) finishes at 10{,}194 on instance 37.
\texttt{pred} access also cuts solver wall-time by a factor of $3$--$8$ (6.7--22.6\,s vs.\ 48.1--54.2\,s) at the cost of roughly $2{\times}$ more AI tokens, because graph-derived endpoint sizes let agents commit to short single-pass simulated-annealing calls instead of long branch-and-bound runs.

\textbf{Solver-choice analysis.}
\Cref{tab:end-user-solver-table} reports the model and solver each agent settled on.
The bare configurations scatter across HiGHS, Gurobi, OR-tools CP-SAT, dwave-neal / dwave-samplers SA, dwave-tabu, and ad-hoc local-search heuristics; the misses concentrate on the integer-programming and quadratic-mixed-integer routes whose linear relaxations are weak on dense signed-weight cut maximization.
With \texttt{pred}, agents recognize the instance as a signed-weight cut on a dense graph, follow the catalog through the spin-glass / QUBO family, and run simulated annealing (or, on a single instance, a multi-replica parallel-tempering variant) on the Ising form on every run.
The catalog therefore acts as a variance reducer for solver selection: it removes the inter-run modelling lottery and consistently routes agents to a reformulation whose endpoint solver dominates within the wall-time budget.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\linewidth]{figures/end_user_scatter.pdf}
\caption{Per-run quality vs.\ solver wall-time across all 40 benchmark runs.
Each marker is one (configuration, instance) pair; the y-axis shows the gap from the per-instance best objective achieved by any configuration on that instance, so the dashed line at $0\%$ is ``matched the best.''
The y-axis is broken between $-3.5\%$ and $-8\%$ to keep the main cluster legible alongside the four catastrophic misses.
Catalog-enabled runs (\textcolor[HTML]{1f77b4}{\textbf{+pred}}, \textcolor[HTML]{2ca02c}{\textbf{+pred+web}}) sit on the upper-left ``fast and on-target'' line; the bare baselines (\textcolor[HTML]{d62728}{\textbf{Bare AI}}, \textcolor[HTML]{ff7f0e}{\textbf{Bare AI+web}}) spread further right and account for three of the four misses in the lower panel.}
\label{fig:end-user-scatter}
\end{figure}
...
\paragraph{Setup.}
A Claude Opus~4.7 agent (\texttt{claude-opus-4-7}, dispatched via Claude Code's general-purpose subagent and recorded in each run's \texttt{meta.json}) receives an engineer-voice prompt: a HR manager splits a 150-person org into two product teams to maximize total cross-team value from a dense signed pairwise survey (${\approx}\,5300$ entries, density ${\approx}\,0.48$, mix of positive and negative scores).
