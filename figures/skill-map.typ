...
#canvas(length: 0.45cm, {
  ...
  // Inset 1.65 from right edge so c4x boxes do not overflow
  let c1x = 16.8
  let c4x = 28.6
  let gap = (c4x - c1x) / 3
  let c2x = c1x + gap
  ...
  let skill(pos, label, name-id, mentor: false) = {
    ...
    rect((x - 1.6, y - 0.34), (x + 1.6, y + 0.34),
      radius: 3pt, fill: bg, stroke: sk, name: name-id)
    content(name-id, text(6.5pt, fill: fg, raw(label)))
  }

  // Root spec node
  let rx = (c1x + c4x) / 2
  let ry = pa-header-y - 0.7
  rect((rx - 2.8, ry - 0.55), (rx + 2.8, ry + 0.55),
