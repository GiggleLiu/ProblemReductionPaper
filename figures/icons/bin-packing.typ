...
#canvas({
  ...
  let bin(cx, items) = {
    let bw = 0.42       // bin inner width
    let item-h = 0.28   // each item height
    let bottom = -0.7
    let wall-h = 1.2    // total wall height
    let wall = (paint: black.lighten(10%), thickness: 0.9pt)
    ...
    for (i, it) in items.enumerate() {
      ...
      let y1 = y0 + item-h
      rect((cx - bw / 2 + 0.02, y0 + 0.02), (cx + bw / 2 - 0.02, y1 - 0.02),
        fill: col,
  ...
  bin(-0.7, ((rgb("#83379d"), [4]), (rgb("#59a14f"), [6])))
