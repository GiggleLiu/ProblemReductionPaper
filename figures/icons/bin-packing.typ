...
#canvas({
  ...
  let bin(cx, items) = {
    let bw = 0.52       // bin inner width
    let item-h = 0.34   // each item height
    let bottom = -0.85
    let wall-h = 1.5    // total wall height
    ...
    for (i, it) in items.enumerate() {
      ...
      rect((cx - bw / 2 + 0.025, y0 + 0.025), (cx + bw / 2 - 0.025, y1 - 0.025),
        ...
        radius: 0.05)
      content((cx, (y0 + y1) / 2),
        text(8pt, weight: "bold", fill: white, label))
    }
  }

  // Bin 1: green 6 on top, purple 4 below.
  bin(-0.82, ((rgb("#83379d"), [4]), (rgb("#59a14f"), [6])))
